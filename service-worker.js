const CACHE_NAME = 'food-safety-scanner-v6-step-scan-simple';
const ASSETS = [
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.endsWith('/data/recalls.json') || url.pathname.endsWith('/recalls.json')) {
...
async function networkFirstData(request) {
  ...
  try {
    ...
    const cache = await caches.open(CACHE_NAME);
    return (await cache.match(canonical)) || Promise.reject(error);
  }
}
